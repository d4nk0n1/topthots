...
if ( isset( $_POST['submit'] ) ) {
	// let's rock and roll
	$nonce = $_POST['wptouch2-nonce'];
	if ( !wp_verify_nonce( $nonce, 'wptouch2-nonce' ) ) {
		_e( "Nonce Failure", "affil4you_libs_wptouch2" );
		die;
	}

	if ( !current_user_can( 'manage_options' ) ) {
		_e( "Security failure.  Please log in again.", "affil4you_libs_wptouch2" ); 
		die;
	}
...
//
	...
	if ( isset( $_POST['wptouch2-language'] ) ) {
		$a['wptouch2-language'] = $_POST['wptouch2-language'];
	} else {
		$a['wptouch2-language'] = "auto";	
	}
	...
	$values = serialize($a);
	update_option('bnc2_iphone_pages', $values);
} elseif ( isset( $_POST['reset'] ) ) {
	update_option( 'bnc2_iphone_pages', false );
}
